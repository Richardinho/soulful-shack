...
module.exports = function(express, config, recordService, userService, multer) {
	...
	apiRouter.get('/records/:page/:minCost/:userRating/:stars/:sortby', function (request, response) {
		recordService.getRecordSummaries(
			...
			request.params.sortby).then( data => {
				response.json(data);
			}, error => {
					console.log(error);
					response.send([{
	...
	apiRouter.get('/record/:id', function (request, response) {
		recordService.getRecord(request.params.id).then( data => {
			response.json(data);
		}, error => {
   ...
   */
	...
	apiRouter.post('/signin', function (request, response){
		userService.retrieveUser(request.body.email, request.body.password).then( user => {
			response.json({ success : true, user : user });
		}).catch( message => {
