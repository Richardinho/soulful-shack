+(function (app) {
	...
	app.config([
		...
		function ($stateProvider) {
			$stateProvider.state("records.summaries", {
				url: "/summaries",
				resolve: {
					/* filters and sort options used by both refinements and pagination */
					refinements: function (serverData, createRefinements) {
						return createRefinements(
							serverData.data.filterdata,
							serverData.data.PaginationData.current
						);
					},
				},
				views: {
					...
					pagination: {
						...
						controller: [
							...
							function (
								...
								$scope.pagination = createPaginationLinks(
									serverData.data.PaginationData
								);
								...
								$scope.changePage = function (requestedPage, enabled) {
									if (enabled) {
										if (requestedPage == "prev") {
											requestedPage =
												parseInt(serverData.data.PaginationData.current, 10) -
												1;
										...
										$state.go(
											"records.summaries",
											createUrlParamsFromFilters(refinements)
										);
									}
								};
							},
